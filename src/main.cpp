...
#if defined(NDEBUG)
#error "Bouygues cannot be compiled without assertions."
#endif
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (tx.IsCoinStake())
        // return state.DoS(100, error("AcceptToMemoryPool: coinstake as individual tx"),REJECT_INVALID, "coinstake");
        return false;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    CAmount nExpectedMint = GetBlockValue(pindex->pprev->nHeight > Params().LAST_POW_BLOCK() ? pindex->pprev->nHeight + 1 : pindex->pprev->nHeight);
    if (block.IsProofOfWork())
        nExpectedMint += nFees;
