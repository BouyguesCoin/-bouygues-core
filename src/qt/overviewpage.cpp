...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::BYG)
    {
    }
...
void OverviewPage::updatePrivatesendProgress()
{
    ...
    QString strAnonymizeBygAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeBygAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeBygAmount = strAnonymizeBygAmount.remove(strAnonymizeBygAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBygAmount + " / " + tr("%n Rounds", "", nPrivatesendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeBygAmount * COIN) nMaxToAnonymize = nAnonymizeBygAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeBygAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeBygAmount));
        strAnonymizeBygAmount = strAnonymizeBygAmount.remove(strAnonymizeBygAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeBygAmount + " / " + tr("%n Rounds", "", nPrivatesendRounds);
    } else {
